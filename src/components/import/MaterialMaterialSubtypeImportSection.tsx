...
import { Link2, Upload, CheckCircle, AlertCircle, Info } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { parseMaterialMaterialSubtypeSQL, importMaterialMaterialSubtypes, MySQLMaterialMaterialSubtypeRecord } from "@/utils/import/materialMaterialSubtypeImporter";

export const MaterialMaterialSubtypeImportSection: React.FC = () => {
  const [sqlData, setSqlData] = useState('');
  const [parsedRecords, setParsedRecords] = useState<MySQLMaterialMaterialSubtypeRecord[]>([]);
  ...
  const handleParseSql = () => {
    try {
      const records = parseMaterialMaterialSubtypeSQL(sqlData);
      setParsedRecords(records);
      
      toast({
        title: "SQL parsad framgångsrikt",
        description: `Hittade ${records.length} material-subtyp-kopplingar att importera`,
      });
    } catch (error) {
  ...
  const handleImportRecords = async () => {
    ...
    try {
      const result = await importMaterialMaterialSubtypes(parsedRecords, (progress) => {
    ...
    <Card className="bg-white/10 backdrop-blur-md border-white/20">
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-white">Material-Subtyp Import</CardTitle>
        </div>
        <CardDescription className="text-slate-300">
          Importera kopplingar mellan material och subtyper från MySQL-export.
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <Textarea
            ...
            placeholder="Klistra in MySQL INSERT statement för material_materialsubtype-tabellen här..."
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            Importera till material_materialsubtype
          </Button>
        </div>
              ...
              {parsedRecords.slice(0, 10).map((record, index) => (
                <div key={index} className="bg-black/30 rounded p-2 text-xs grid grid-cols-2 gap-x-2">
                  <div className="truncate">
                    ...
                    <span className="text-cyan-400">{record.materialid}</span>
                  </div>
                  <div className="truncate">
                    <span className="text-slate-400">Subtyp ID: </span>
                    <span className="text-amber-400">{record.subtypeid}</span>
                  </div>
                </div>
              ))}
        ...
        {importResult && (
          <div className="bg-black/20 rounded p-4">
            <h4 className="text-white font-semibold mb-2 flex items-center gap-2">
              ...
              Importresultat - material_materialsubtype-tabellen
            </h4>
