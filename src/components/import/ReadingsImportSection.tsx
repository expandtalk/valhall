...
import { Book, Upload, CheckCircle, AlertCircle, Info } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { parseReadingsSQL, importReadings, MySQLReadingRecord } from "@/utils/import/readingsImporter";

export const ReadingsImportSection: React.FC = () => {
  const [sqlData, setSqlData] = useState('');
  const [parsedReadings, setParsedReadings] = useState<MySQLReadingRecord[]>([]);
  ...
  const handleParseSql = () => {
    try {
      const readings = parseReadingsSQL(sqlData);
      setParsedReadings(readings);
      
      toast({
        title: "SQL parsad framgångsrikt",
        description: `Hittade ${readings.length} läsningar att importera`,
      });
    } catch (error) {
  ...
  const handleImportReadings = async () => {
    ...
    try {
      ...
      for (let i = 0; i < parsedReadings.length; i += batchSize) {
        ...
        const result = await importReadings(batch);
        ...
        setImportProgress(((i + batch.length) / parsedReadings.length) * 100);
      }
    ...
    <Card className="bg-white/10 backdrop-blur-md border-white/20">
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-white">Readings Import</CardTitle>
        </div>
        <CardDescription className="text-slate-300">
          Importera läsningar av runinskrifter från MySQL-export
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <Textarea
            ...
            placeholder="Klistra in MySQL INSERT statement för readings-tabellen här..."
        ...
        <div className="flex gap-3">
          ...
          <Button
            ...
            disabled={parsedReadings.length === 0 || isImporting}
        ...
        {parsedReadings.length > 0 && (
          <div className="bg-black/20 rounded p-4">
            ...
            <Badge className="bg-blue-500 text-white border-0 mb-2">
              {parsedReadings.length} läsningar hittade
            </Badge>
            
            <div className="text-sm text-slate-300 space-y-1 max-h-32 overflow-y-auto">
              {parsedReadings.slice(0, 5).map((reading, index) => (
                <div key={index} className="bg-black/30 rounded p-2">
                  <div className="text-amber-400">Typ: {reading.reading}</div>
                  <div className="text-slate-300 truncate">
                    Text: {reading.text || 'Ingen text'}
                  </div>
                  {reading.teitext && (
                    <div className="text-blue-400 text-xs truncate">
                      TEI: {reading.teitext.substring(0, 50)}...
                    </div>
                  )}
                </div>
              ))}
              {parsedReadings.length > 5 && (
                <div className="text-slate-400 text-center">
                  ...och {parsedReadings.length - 5} till
                </div>
              )}
            </div>
          </div>
