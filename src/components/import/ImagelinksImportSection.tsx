...
import { Image as ImageIcon, Upload, CheckCircle, AlertCircle, Info } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { parseImageLinksSQL, importImageLinks, MySQLImageLinkRecord } from "@/utils/import/imagelinksImporter";

export const ImagelinksImportSection: React.FC = () => {
  const [sqlData, setSqlData] = useState('');
  const [parsedRecords, setParsedRecords] = useState<MySQLImageLinkRecord[]>([]);
  ...
  const handleParseSql = () => {
    try {
      const records = parseImageLinksSQL(sqlData);
      setParsedRecords(records);
      
      toast({
        title: "SQL parsad framgångsrikt",
        description: `Hittade ${records.length} bildlänkar att importera`,
      });
    } catch (error) {
  ...
  const handleImportRecords = async () => {
    ...
    try {
      const result = await importImageLinks(parsedRecords, (progress) => {
    ...
    <Card className="bg-white/10 backdrop-blur-md border-white/20">
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-white">Bildlänksimport</CardTitle>
        </div>
        <CardDescription className="text-slate-300">
          Importera bildlänkar från MySQL-export (imagelinks-tabellen).
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <Textarea
            ...
            placeholder="Klistra in MySQL INSERT statement för imagelinks-tabellen här..."
        ...
        {parsedRecords.length > 0 && (
          <div className="bg-black/20 rounded p-4">
            ...
            <Badge className="bg-cyan-500 text-white border-0 mb-2">
              {parsedRecords.length} bildlänkar hittade
            </Badge>
              ...
              {parsedRecords.slice(0, 10).map((record, index) => (
                <div key={index} className="bg-black/30 rounded p-2 text-xs grid grid-cols-2 gap-x-2">
                  <div className="truncate">
                    <span className="text-slate-400">Objekt ID: </span>
                    <span className="text-cyan-400">{record.objectid}</span>
                  </div>
                  <div className="truncate">
                    <span className="text-slate-400">Länk: </span>
                    <span className="text-amber-400">{record.imagelink}</span>
                  </div>
                </div>
              ))}
        ...
        {isImporting && (
          <div className="bg-black/20 rounded p-4">
            <h4 className="text-white font-semibold mb-2">Importerar bildlänkar...</h4>
        ...
        {importResult && (
          <div className="bg-black/20 rounded p-4">
            <h4 className="text-white font-semibold mb-2 flex items-center gap-2">
              ...
              Importresultat - imagelinks-tabellen
            </h4>
