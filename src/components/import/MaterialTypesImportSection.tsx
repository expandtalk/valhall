...
import { Palette, Upload, CheckCircle, AlertCircle, Info } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { parseMaterialTypesSQL, importMaterialTypes, MySQLMaterialTypeRecord } from "@/utils/import/materialTypesImporter";

export const MaterialTypesImportSection: React.FC = () => {
  const [sqlData, setSqlData] = useState('');
  const [parsedRecords, setParsedRecords] = useState<MySQLMaterialTypeRecord[]>([]);
  ...
  const handleParseSql = () => {
    try {
      const records = parseMaterialTypesSQL(sqlData);
      setParsedRecords(records);
      
      toast({
        title: "SQL parsad framgångsrikt",
        description: `Hittade ${records.length} materialtyper att importera`,
      });
    } catch (error) {
  ...
  const handleImportRecords = async () => {
    ...
    try {
      console.log('🚀 Starting direct import to materialtypes table...');
      
      const result = await importMaterialTypes(parsedRecords);
    ...
    <Card className="bg-white/10 backdrop-blur-md border-white/20">
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-white">Materialtyper Import</CardTitle>
        </div>
        <CardDescription className="text-slate-300">
          Importera materialtyper från MySQL-export (sten, trä, metall, etc.)
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <Textarea
            ...
            placeholder="Klistra in MySQL INSERT statement för materialtypes-tabellen här..."
        ...
        {parsedRecords.length > 0 && (
          <div className="bg-black/20 rounded p-4">
            ...
            <Badge className="bg-amber-500 text-white border-0 mb-2">
              {parsedRecords.length} materialtyper hittade
            </Badge>
              ...
              {parsedRecords.slice(0, 10).map((record, index) => (
                <div key={index} className="bg-black/30 rounded p-2">
                  <div className="text-amber-400 text-xs">
                    Typ: {record.materialtype}
                  </div>
                  <div className="text-blue-400 text-xs">
                    Språk: {record.lang}
                  </div>
                  <div className="text-slate-400 text-xs">
                    ID: {record.materialtypeid.substring(0, 12)}...
                  </div>
                </div>
              ))}
        ...
        {isImporting && (
          <div className="bg-black/20 rounded p-4">
            <h4 className="text-white font-semibold mb-2">Importerar till materialtypes...</h4>
        ...
        {importResult && (
          <div className="bg-black/20 rounded p-4">
            <h4 className="text-white font-semibold mb-2 flex items-center gap-2">
              ...
              Importresultat - materialtypes-tabellen
            </h4>
