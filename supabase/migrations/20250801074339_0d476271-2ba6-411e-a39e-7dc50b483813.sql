-- Uppdatera search-funktionen för att hantera både primary_signum och alternative_signum korrekt
...
BEGIN
  ...
  SELECT 
    ri.id, 
    COALESCE(ri.primary_signum, ri.signum) as signum, -- ✅ Använd primary_signum som huvudsignum
  ...
  WHERE 
    ...
    -- ✅ Primary signum search (prioritet)
    ...
    -- Legacy signum search with normalized format
  ...
  ORDER BY 
    -- Prioritize exact primary signum matches first
    CASE 
      WHEN LOWER(COALESCE(ri.primary_signum, ri.signum)) = LOWER(p_search_term) THEN 1
      WHEN LOWER(ri.name) = LOWER(p_search_term) THEN 2
      WHEN LOWER(ri.name_en) = LOWER(p_search_term) THEN 3
      ...
      ELSE 8
    END,
    COALESCE(ri.primary_signum, ri.signum) ASC; -- ✅ Sortera på primary_signum
END;
$function$;