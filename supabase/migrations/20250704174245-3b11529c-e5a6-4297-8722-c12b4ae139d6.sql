-- Migration: Fix geographical classification errors
...
-- These should be removed or corrected since Bergen is a Norwegian city
UPDATE runic_inscriptions 
SET municipality = NULL, 
    province = NULL,
    notes = COALESCE(notes || '; ', '') || 'Geographic data corrected: Bergen removed from Swedish inscriptions'
...
SET landscape = 'Gotland',
    ...
    municipality = CASE 
      WHEN parish LIKE '%Visby%' THEN 'Gotland'
      ELSE 'Gotland'
    END
WHERE country = 'Sweden' 
...
INSERT INTO migration_log (migration_name, records_affected, description, executed_at)
VALUES 
  ('fix_geographical_classification', 
   (SELECT COUNT(*) FROM runic_inscriptions WHERE landscape IS NOT NULL), 
   'Fixed geographical classification: moved landscapes from province to landscape field, corrected Bergen entries, updated Norwegian provinces',
   NOW()
  );
